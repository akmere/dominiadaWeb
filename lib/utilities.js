...
export function getMatchLinkElement(matchId, text = 'Details') {
...
export function getDateElement(date) {
  try {
    ...
    return (<div style={{ textAlign: 'center' }}><p>{bob.getDate().toString().padStart(2, '0')}/{(bob.getMonth() + 1).toString().padStart(2, '0')}/{bob.getFullYear()}</p><p>{bob.getHours().toString().padStart(2, '0')}:{bob.getMinutes().toString().padStart(2, '0')}:{bob.getSeconds().toString().padStart(2, '0')}</p></div>)
  }
  catch (e) {
    return '';
  }
