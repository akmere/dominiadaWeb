...
export default async function Home() {
  ...
  let matchesRows = matches.map((row, index) => ({ id: index, position: index + 1, match: getMatchLinkElement(row.pk), reds: getFullTeamElement(appearances, row.pk, 1), blues: getFullTeamElement(appearances, row.pk, 2), ...row, ddate: getDateElement(row.ddate) }));
  let matchesColumns = [];
